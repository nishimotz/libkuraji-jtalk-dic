...
- `.github/workflows/release-dic.yml`: `v*` タグを push したときのみ実行。上記と同じ手順でビルドし、`build/dic/` を zip 化して GitHub Releases に公開する。タグ名がそのままファイル名（`jtalk-dic-<tag>.zip`）とリリースに使われる。
...
これをトリガーに CI が辞書一式をビルドし、`jtalk-dic-v0.1.0.zip` と `jtalk-dic-v0.1.0.zip.sha256` を添付した GitHub Release を自動作成する。
